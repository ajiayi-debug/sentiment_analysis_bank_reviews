...
If users want to change the whole set of sentiment_data and summary, users will need to re-run dataset.py to webscrape, clean and store the data as well as scrape_all.ipynb to get a new set of sentiment_data and summary data. Users need to remember to update all_databases.sql (or create a new .sql file for safety reasons)in order for other users to obtain the data. Users can do so by running `docker exec database_docker sh -c 'exec mysqldump -u root --password=MYSQL12345 --all-databases' > all_databases.sql` . (replace all_databases.sql with the new .sql file if new file is used)
